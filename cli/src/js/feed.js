(() => {
  ...
  var require_she = __commonJS({
    "node_modules/insane/she.js"(exports, module) {
      ...
      function unescapeHtml(html5) {
        return html5 == null ? "" : String(html5).replace(rescaped, unescapeHtmlChar);
      }
  ...
  var require_parser = __commonJS({
    "node_modules/insane/parser.js"(exports, module) {
      ...
      function parser2(html5, handler) {
        ...
        function parseTag() {
          if (html5.substr(0, 4) === "<!--") {
            parseComment();
          } else if (rtagend.test(html5)) {
            parseEdge(rend, parseEndTag);
          } else if (rtag.test(html5)) {
            parseEdge(rstart, parseStartTag);
          }
          parseTagDecode();
        }
        function parseEdge(regex, parser3) {
          var match = html5.match(regex);
          if (match) {
            html5 = html5.substring(match[0].length);
            match[0].replace(regex, parser3);
            chars = false;
          }
        }
        function parseComment() {
          var index = html5.indexOf("-->");
          if (index >= 0) {
            if (handler.comment) {
              handler.comment(html5.substring(4, index));
            }
            html5 = html5.substring(index + 3);
            chars = false;
          }
        }
        function parseTagDecode() {
          ...
          var index = html5.indexOf("<");
          if (index >= 0) {
            text = html5.substring(0, index);
            html5 = html5.substring(index);
          } else {
            text = html5;
            html5 = "";
          }
  ...
  var StubServer = class {
    constructor() {
      ...
      this.index = {
        ...
        order: "newest content first",
        // feeds use "newest content first", stories use "oldest content first"
...
* the passage of time.
  ...
  function unescape(html5) {
    return html5.replace(unescapeTest, (_, n3) => {
      n3 = n3.toLowerCase();
      if (n3 === "colon")
        return ":";
      if (n3.charAt(0) === "#") {
...
`;
  ...
  var Marked = class {
    ...
    #parseMarkdown(lexer2, parser2) {
      return (src, options2) => {
        ...
        if (opt.async) {
          return Promise.resolve(opt.hooks ? opt.hooks.preprocess(src) : src).then((src2) => lexer2(src2, opt)).then((tokens) => opt.walkTokens ? Promise.all(this.walkTokens(tokens, opt.walkTokens)).then(() => tokens) : tokens).then((tokens) => parser2(tokens, opt)).then((html5) => opt.hooks ? opt.hooks.postprocess(html5) : html5).catch(throwError);
        }
        try {
          if (opt.hooks) {
  ...
  function AnyCard({ card, cardType, stackIndex, primary, visible, children }) {
    ...
    return html2`<div style=${z3} class="card ${cardType}-card any-card ${stackIndex ? "stacked" : ""} ${card.extraClass.join(" ")}">
        ${children}
    </div>`;
  }
  function TitleCard({ card, stackIndex, primary, visible }) {
    return html2`<${AnyCard} card=${card} cardType="title" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        <h1>${card.title ?? card.id}</h1>
    </${AnyCard}>`;
  }
  function MarkdownCard({ card, stackIndex, primary, visible }) {
    return html2`<${AnyCard} card=${card} cardType="markdown" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function HtmlCard({ card, stackIndex, primary, visible }) {
    return html2`<${AnyCard} card=${card} cardType="html" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function ImageCard({ card, stackIndex, primary, visible }) {
    return html2`<${AnyCard} card=${card} cardType="image" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function PanDownCard({ card, stackIndex, primary, visible }) {
    ...
    return html2`<${AnyCard} card=${card} cardType="pan-down" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        <img src=${card.imageUrl} alt=${card.alt} title=${card.title}/>
    </${AnyCard}>`;
  }
  function PanLeftCard({ card, stackIndex, primary, visible }) {
    ...
    return html2`<${AnyCard} card=${card} cardType="pan-left" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        <img src=${card.imageUrl} alt=${card.alt} title=${card.title}/>
    </${AnyCard}>`;
  }
  function PanUpCard({ card, stackIndex, primary, visible }) {
    ...
    return html2`<${AnyCard} card=${card} cardType="pan-up" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function PanRightCard({ card, stackIndex, primary, visible }) {
    ...
    return html2`<${AnyCard} card=${card} cardType="pan-right" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        <img style=${initialStyle} src=${card.imageUrl} alt=${card.alt} title=${card.title}/>
    </${AnyCard}>`;
  }
  function AnimatedImageCard({ card, primary, visible, stackIndex }) {
    ...
    let images = imagesToCycleThrough.map((imageUrl, index) => {
      return html2`<img src=${imageUrl} alt=${card.alt} title=${card.title} style="display: ${index === 0 ? "block" : "none"};"/>`;
    });
    return html2`<${AnyCard} card=${card} cardType="animated-image" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        ${images}
    </${AnyCard}>`;
  }
  function AnimatedTextCard({ card, primary, visible, stackIndex }) {
    ...
    let textSeparated = textToAnimate.split("").map((char, index) => {
      return html2`<span style="opacity: 0;">${char}</span>`;
    });
    return html2`<${AnyCard} card=${card} cardType="animated-text" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function VideoCard({ card, primary, visible, stackIndex }) {
    ...
    return html2`<${AnyCard} card=${card} cardType="video" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function ErrorCard({ card, stackIndex, primary, visible }) {
    return html2`<${AnyCard} card=${card} cardType="error" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
  ...
  function StackedCard({ card, primary, visible, stackIndex }) {
    return html2`<${AnyCard} card=${card} cardType="stack" stackIndex=${stackIndex} primary=${primary} visible=${visible}>
        ${card.stack.map((c3, index) => {
      let cardClass = typeToCardClass(c3.type);
      let newStackIndex = (stackIndex ?? 0 * 100) + index + 1;
      return html2`<${cardClass} card=${c3} primary=${primary} visible=${visible} stackIndex=${newStackIndex} />`;
    })}
    </${AnyCard}>`;
  }
  function RenderedContent({ content, primary, visible }) {
    ...
    return html2`<div class="rendered-content">
    ...
    */
    ...
    render() {
      ...
      if (node && (this.state.primary || this.state.visible)) {
        maybeContent = html3`<${RenderedContent} content=${node} primary=${this.state.primary} visible=${this.state.visible}/>`;
      }
      return html3`<div class="frame ${frameClass}">
            ${maybeContent}
        </div>`;
    }
  };
  ...
  var App = class extends b {
    ...
    render() {
      ...
      let items = this.state.index.contentIds.map((id, n3) => {
        ...
        return html4`<${VisibilityTriggerFrame} data=${this.data} order=${n3} id=${id} onPrimary=${select}/>`;
      });
      return html4`<div class="primary-card">
            <div class="content">
